...
async function removeRow(_id) {
  ...
  try {
    await POST(
      { username: lastUsername, course: crs, type: typ, grade: grd },
      "removeGrade"
    );
  } catch (error) {
    console.log(error);
      ...
      <>
        <td>
          <input
            type="number"
            maxLength="3"
            onInput={maxLengthCheck.bind(this, "vg-" + id + "-g")}
            id={"vg-" + id + "-g"}
            name={grade}
        ...
        <td onClick={updateRow.bind(this, id)}>
          <Button variant="primary" size="xs" style={{ color: "white" }} n>
            Update
          </Button>
        </td>
        <td onClick={removeRow.bind(this, id)}>
          <Button variant="primary" size="xs" style={{ color: "white" }}>
            Remove
          </Button>
        </td>
      </>
    ...
    return (
      <>
        <tr id={"vg-" + id}>
          ...
          <td id={"vg-" + id + "-c"} className="text-primary fw-bold">
            {course}
          </td>
      ...
      <Card border="light" className="shadow-sm mb-4">
        <Card.Body className="pb-0">
          ...
          >
            ...
            <tbody>
              {token.map((pt) => (
                <TableRow key={`view-grades-${pt.id}`} {...pt} />
              ))}
            </tbody>
          </Table>
        </Card.Body>
